...
test('decode stream', function (t) {
  t.test('decode version', function (t) {
    ...
    stream.once('data', function (message) {
      ...
      t.same(message.checksum, Buffer.from('d33fa729', 'hex'), 'correct checksum')
      t.same(message.payload.version, 31900, 'correct version')
      t.same(message.payload.services, Buffer.from('0100000000000000', 'hex'), 'correct services')
      t.same(message.payload.timestamp, 1292899814, 'correct timestamp')
      t.same(message.payload.receiverAddress.services, Buffer.from('0100000000000000', 'hex'), 'correct receiver address services')
      ...
      t.same(message.payload.senderAddress.services, Buffer.from('0100000000000000', 'hex'), 'correct sender address services')
      ...
      t.same(message.payload.nonce, Buffer.from('dd9d202c3ab45713', 'hex'), 'correct nonce')
    ...
    stream.write(Buffer.from('f9beb4d976657273696f6e000000000056000000d33fa7299c7c00000100000000000000e615104d00000000010000000000000000000000000000000000ffff0a000001208d010000000000000000000000000000000000ffff0a000002208ddd9d202c3ab45713005581010000', 'hex'))
  })
  ...
  t.test('decode verack', function (t) {
    ...
    stream.write(Buffer.from('f9beb4d976657261636b000000000000000000005df6e0e2', 'hex'))
  })
  ...
  t.test('decode addr', function (t) {
    ...
    stream.once('data', function (message) {
      ...
      t.same(message.payload[0].services, Buffer.from('0100000000000000', 'hex'), 'correct services')
      t.same(message.payload[0].address, '10.0.0.1', 'correct IP')
      t.same(message.payload[0].port, 8333, 'correct port')
      t.end()
    })
    stream.write(Buffer.from('f9beb4d96164647200000000000000001f000000ed52399b01e215104d010000000000000000000000000000000000ffff0a000001208d', 'hex'))
  })
  ...
  t.test('decode tx', function (t) {
    ...
    stream.once('data', function (message) {
      ...
      t.same(message.payload.ins[0].hash, Buffer.from('6dbddb085b1d8af75184f0bc01fad58d1266e9b63b50881990e4b40d6aee3629', 'hex'), 'correct input hash')
      t.same(message.payload.ins[0].index, 0, 'correct input index')
      t.same(message.payload.ins[0].script, Buffer.from('', 'hex'), 'correct input index')
    ...
    stream.write(Buffer.from('', 'hex'))
  })
  ...
  t.test('invalid magic', function (t) {
    ...
    stream.write(Buffer.from('ffffffff76657261636b000000000000000000005df6e0e2', 'hex'))
  })
  ...
  t.test('invalid string padding', function (t) {
    ...
    stream.write(Buffer.from('f9beb4d976657261636b00ff00000000000000005df6e0e2', 'hex'))
  })
  ...
  t.test('invalid command', function (t) {
    ...
    stream.write(Buffer.from('f9beb4d9616263640000000000000000000000005df6e0e2', 'hex'))
  })
  ...
  t.test('invalid checksum', function (t) {
    ...
    stream.write(Buffer.from('f9beb4d970696e67000000000000000008000000ffffffff0123456789abcdef', 'hex'))
  })

  t.test('buffering', function (t) {
    var stream = bp.createDecodeStream()
    var data = Buffer.from('f9beb4d970696e67000000000000000008000000137ad6630123456789abcdef', 'hex')
    stream.once('data', function (message) {
      t.true(message, 'got data')
      t.same(message.magic, 3652501241, 'correct magic')
